...
nombre_carpeta = os.path.join("C:\\Users\\alejandro.berzal\\Desktop\\DATA SCIENCE\\NEW PO\\data\\" +dia)
if not os.path.isdir(nombre_carpeta):
    print(f'No existe la ruta: '+nombre_carpeta+', se crea la carpeta')
    os.mkdir(nombre_carpeta)

# Generamos las carpetas correspondientes para guardar los archivos
nombre_carpeta2 = os.path.join("C:\\Users\\alejandro.berzal\\Desktop\\DATA SCIENCE\\NEW PO\\data\\" +dia+ "\\ERP")
if not os.path.isdir(nombre_carpeta2):
...
inbox = outlook.GetDefaultFolder(6).Folders["test2"]    # Quitar selección de carpeta
...
while message:
    ...
    if message.SenderEmailAddress == 'egesdoc@grupotr.es':
        try:
            ...
            df['Nº Transmittal'] = df['subject'].str.extract(r'\((.*?)\)', expand=False)    # Obtenemos a traves del subject el Nº de transmittal que ha sido emitido.
            ...
            df_import = df_import.reindex(['Documento EIPSA', 'Documento Cliente', 'Nº Pedido', 'Título', 'Tipo de documento', 'Crítico'], axis=1)    # Estructuramos los datos del df_import.
            ...
            body = df_final.to_html()    # Volcamos el dataframe DF a HTML para la unión al email.
            ...
            attach = 'C:\\Users\\alejandro.berzal\\Desktop\\DATA SCIENCE\\NEW PO\\RESUMEN - ' + subject_email + '.xlsx'    # Url para la captura del documento.
